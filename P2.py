...
def draw_lines(img, lines, color=[255, 0, 0], thickness=2):
    ...
    lanes = [[0, 0, 0], [0, 0, 0]]  # use lane[2] for count
    for i in lines:
        line = i[0]

        # cv2.line(img, (line[0], line[1]), (line[2], line[3]), [i / 3 for i in color], thickness)
        # continue

        a = lineParam(line)
        #         a.append[1] # for counter
        SLOPE_RANGE = (0.3, 3)
        if a[1] > SLOPE_RANGE[0] and a[1] < SLOPE_RANGE[1]:
            # and line[0] > hcenter and line[2] > hcenter:
            lanes[0][0] += a[0]
            lanes[0][1] += a[1]
            lanes[0][2] += 1

        elif -a[1] > SLOPE_RANGE[0] and -a[1] < SLOPE_RANGE[1]:
            # and line[0] < hcenter and line[2] < hcenter
            lanes[1][0] += a[0]
            lanes[1][1] += a[1]
            lanes[1][2] += 1
